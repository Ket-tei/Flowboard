...
import type { UpdateTemplateItemInput, ReorderTemplateItemsInput, TransitionValue } from "../schemas/template.schema.js";

export async function uploadTemplateItem(
  templateId: number,
  file: { stream: Readable; mimetype: string; filename: string },
  opts: { durationMs: number; transitionType: TransitionValue; transitionDurationMs: number }
) {
  const mime = file.mimetype ?? "application/octet-stream";
  const kind = mimeToMediaType(mime);
  if (!kind) throw new BadRequestError("unsupported media type");
...
export async function reorderTemplateItems(
  ...
  for (let i = 0; i < input.orderedIds.length; i++) {
    if (!setIds.has(input.orderedIds[i])) throw new BadRequestError("invalid item id");
...
export async function updateTemplateItem(
  ...
  if (!row[0] || row[0].templateId !== templateId) throw new NotFoundError("item not found");

  const updates: Partial<{ durationMs: number; transitionType: TransitionValue; transitionDurationMs: number }> = {};

import { mysqlTable, varchar, int, bigint, mysqlEnum, timestamp, primaryKey, decimal, } from "drizzle-orm/mysql-core";
...
export const userFolderAccess = mysqlTable("user_folder_access", {
    userId: bigint("user_id", { mode: "number" }).notNull(),
    folderId: bigint("folder_id", { mode: "number" }).notNull(),
}, (t) => [primaryKey({ columns: [t.userId, t.folderId] })]);
export const userScreenAccess = mysqlTable("user_screen_access", {
    userId: bigint("user_id", { mode: "number" }).notNull(),
    screenId: bigint("screen_id", { mode: "number" }).notNull(),
...
export const userTemplateFolderAccess = mysqlTable("user_template_folder_access", {
    userId: bigint("user_id", { mode: "number" }).notNull(),
    templateFolderId: bigint("template_folder_id", { mode: "number" }).notNull(),
}, (t) => [primaryKey({ columns: [t.userId, t.templateFolderId] })]);
export const userTemplateAccess = mysqlTable("user_template_access", {
    userId: bigint("user_id", { mode: "number" }).notNull(),
    templateId: bigint("template_id", { mode: "number" }).notNull(),

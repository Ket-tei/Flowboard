...
import { userFolderAccess, userScreenAccess, users, } from "../db/schema.js";
import { authPreHandler, requireAdmin } from "../plugins/require-auth.js";
export async function registerUserRoutes(app) {
    app.get("/users", { preHandler: authPreHandler }, async (request, reply) => {
    ...
    app.get("/users/:id/access", { preHandler: authPreHandler }, async (request, reply) => {
        const err = requireAdmin(request, reply);
        if (err)
            return err;
    ...
    app.post("/users", { preHandler: authPreHandler }, async (request, reply) => {
        const err = requireAdmin(request, reply);
        if (err)
            return err;
    ...
    app.patch("/users/:id", { preHandler: authPreHandler }, async (request, reply) => {
        const err = requireAdmin(request, reply);
        if (err)
            return err;
        const id = Number(request.params.id);
        ...
        if (body.folderIds !== undefined || body.screenIds !== undefined) {
            ...
            if (role === "USER") {
                ...
                const screenIds = body.screenIds ?? [];
                if (folderIds.length) {
                    await db.insert(userFolderAccess).values(folderIds.map((folderId) => ({ userId: id, folderId })));
                }
                if (screenIds.length) {
                    await db.insert(userScreenAccess).values(screenIds.map((screenId) => ({ userId: id, screenId })));
                }
            }
        }
        return { ok: true };
    });
    app.delete("/users/:id", { preHandler: authPreHandler }, async (request, reply) => {
        const err = requireAdmin(request, reply);
        if (err)
            return err;
        const id = Number(request.params.id);

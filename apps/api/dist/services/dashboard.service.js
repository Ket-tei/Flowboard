...
import { users, folders, screens, screenItems, dashboardLayouts, userFolderAccess, userScreenAccess, } from "../db/schema.js";
async function getCommonStatsForAdmin() {
    ...
    const [durRes] = await db
        .select({ total: sql `COALESCE(SUM(${screenItems.durationMs}), 0)` })
        .from(screenItems);
...
async function getCommonStatsForUser(userId) {
    ...
    const visibleScreens = allScreens.filter((s) => folderIdSet.has(s.folderId) || directScreenIds.has(s.id));
...
export async function getDashboardData(userId, role) {
    ...
    if (layoutRow.length > 0) {
        try {
            layout = JSON.parse(layoutRow[0].layoutJson);
        }
        catch {
            layout = [];
        }

...
export function buildTemplateTree(folderRows, templatesByFolder, parentId) {
    return folderRows
...
export async function bumpTemplateRevision(templateId) {
    await db.update(templates).set({ revision: sql `${templates.revision} + 1` }).where(eq(templates.id, templateId));
}
export async function getTemplateTree(userId, role) {
...
export async function updateTemplateFolder(folderId, input) {
    const updates = {};
    if (input.name != null)
...
export async function createTemplate(folderId, input) {

...
export async function createScreen(folderId, input, displayMode = "QUICK") {
    const f = await db.select().from(folders).where(eq(folders.id, folderId)).limit(1);
    if (!f[0])
        throw new ScreenError("folder not found", 404);
...
export async function getScreenDetail(screenId) {
    ...
    if (!scr[0])
        throw new ScreenError("screen not found", 404);
...
export async function updateScreen(screenId, input) {
    const updates = {};
    if (input.name != null)
...
export async function uploadItem(screenId, file, durationMs) {
    const mime = file.mimetype ?? "application/octet-stream";
    const kind = mimeToMediaType(mime);
    if (!kind)
        throw new ScreenError("unsupported media type", 400);
...
export async function reorderItems(screenId, input) {
...
export async function updateItem(screenId, itemId, input) {
...
export async function deleteItem(screenId, itemId) {
    const row = await db.select().from(screenItems).where(eq(screenItems.id, itemId)).limit(1);
    if (!row[0] || row[0].screenId !== screenId) {
        throw new ScreenError("item not found", 404);
    }

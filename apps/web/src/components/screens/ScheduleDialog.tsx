...
import { CalendarDays, Eye, MonitorPlay, Plus, X } from "lucide-react";
...
import type { ScreenRow, TemplateWidget } from "@/types/screen.types";
...
export function ScheduleDialog({
  ...
  const [previewItems, setPreviewItems] = useState<PlayerItem[]>([]);
  const [previewWidgets, setPreviewWidgets] = useState<TemplateWidget[]>([]);
  ...
  async function openPreview(slot: ScheduleSlot) {
    try {
      const r = await apiFetch<{
        screen: { name: string };
        items: Array<{ id: number; type: string; durationMs: number; mimeType: string; transitionType?: string }>;
        widgets?: TemplateWidget[];
      }>(`/api/templates/${slot.templateId}`);
      const items: PlayerItem[] = r.items.map((it) => ({
        id: it.id,
        type: it.type,
        durationMs: it.durationMs,
        mimeType: it.mimeType,
        url: apiUrl(`/api/public/templates/${slot.templateId}/media/${it.id}`),
        transitionType: it.transitionType ?? "NONE",
      }));
      setPreviewItems(items);
      setPreviewWidgets(r.widgets ?? []);
      setPreviewTitle(templates.find((t) => t.id === slot.templateId)?.name ?? r.screen.name);
      setPreviewOpen(true);
    } catch {
                  ...
                  <div key={dow} className="flex flex-col gap-2">
                    ...
                    {daySlots.map((slot, idx) => {
                      ...
                      return (
                        ...
                        >
                          <p className="font-medium text-primary truncate pr-6">{tplName}</p>
                          ...
                          <div className="absolute right-1 top-1 hidden group-hover:flex gap-0.5">
                            <button
                              type="button"
                              onClick={() => void openPreview(slot)}
                              className="flex size-4 items-center justify-center rounded-full bg-primary/20 text-primary hover:bg-primary/40"
                            ...
                            <button
                              ...
                              className="flex size-4 items-center justify-center rounded-full bg-destructive/80 text-destructive-foreground"
                            >

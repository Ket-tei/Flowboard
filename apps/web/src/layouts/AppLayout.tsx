      ...
      <nav className={cn("flex flex-1 flex-col gap-0.5 px-2 pb-3", navClassName)}>
        {visible.map((item) => (
          <NavLink
            ...
            className={({ isActive }) =>
              cn(
                "flex items-center gap-3 rounded-lg px-3 py-2.5 text-[13px] font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-border",
                isActive
...
export function AppLayout() {
  ...
  return (
    <div className="bg-muted/40 flex h-svh flex-col overflow-hidden md:flex-row">
      <aside className="bg-card hidden w-64 shrink-0 flex-col border-r border-border shadow-[1px_0_0_0] shadow-border/60 md:flex">
      ...
      <div className="flex min-h-0 min-w-0 flex-1 flex-col bg-background">
        <header className="bg-background/95 flex h-14 shrink-0 items-center gap-3 border-b border-border px-4 backdrop-blur supports-[backdrop-filter]:bg-background/80 md:px-6">
        ...
        <main className="min-h-0 flex-1 overflow-auto">
          <div className="mx-auto w-full max-w-6xl px-4 py-6 pb-10 md:px-6 md:py-8">
            <Outlet />
          </div>
        </main>
      </div>

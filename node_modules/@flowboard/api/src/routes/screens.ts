...
export async function registerScreenRoutes(app: FastifyInstance) {
  ...
  app.get(
    ...
    async (request, reply) => {
      ...
      if (!scr[0]) return reply.status(404).send({ error: "not found" });
  ...
  app.patch(
    ...
    async (request, reply) => {
      ...
      const id = Number((request.params as { id: string }).id);
      if (!Number.isFinite(id)) return reply.status(400).send({ error: "invalid id" });
      const body = request.body as { name?: string; sortOrder?: number; folderId?: number };
  ...
  app.patch(
    ...
    async (request, reply) => {
      ...
      const body = request.body as { durationMs?: number };
